...
from poll.forms import PollVoteManyForm
...
def _render_form(poll, comment, request, csrf_token):
    ...
    return render_to_string('poll/form.html', context)
...
def _render_results(poll, comment, request, csrf_token):
    ...
    return render_to_string('poll/results.html', context)
...
def _evaluate(polls_by_name, comment, request, csrf_token):
    def evaluate(m):
        ...
        show_poll = str(poll.pk) == request.GET.get('show_poll') 
        show_results = (
